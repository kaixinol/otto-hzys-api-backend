...
function isAudioCacheEnabled() {
  const raw = process.env.OTTO_HZYS_CACHE_AUDIO;
...
const cacheAudio = isAudioCacheEnabled();
...
async function getWav() {
  if (!wavPromise) {
    wavPromise = import('node-wav').catch((error) => {
      wavPromise = null;
      throw error;
    });
  }
...
async function renderBuffersToWavBuffer(audioFiles) {
  const decodedParts = await Promise.all(
    audioFiles.map(async ({ buffer, extension }) => {
      const decoded = await decodeAudioBuffer(buffer, extension);
